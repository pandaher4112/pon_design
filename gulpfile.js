...
gulp.task('iconfonts', function (done) {
  gulp.src(['work/icons/svg/*.svg'])
    ...
    .on('glyphs', function (glyphs, options) {
      ...
      gulp.src('work/icons/templates/icon.scss')
      ...
      gulp.src('work/icons/templates/iconfont.scss')
        .pipe(consolidate(engine, consolidateOptions))
        .pipe(rename({ basename: '_icon' }))
        .pipe(gulp.dest('develop/scss/object/component/')); // scssの吐き出し先を指定
      // アイコンフォント用のcssを作成(プレビュー用)
      gulp.src('work/icons/templates/iconfont.css')
        .pipe(consolidate(engine, consolidateOptions))
        .pipe(rename({ basename: 'sample' }))
        .pipe(gulp.dest('develop/icons/')); // scssの吐き出し先を指定
      // アイコンフォント一覧のサンプルHTMLを作成(プレビュー用)
      gulp.src('work/icons/templates/iconfont.html')
        .pipe(consolidate(engine, consolidateOptions))
        .pipe(rename({ basename: 'sample' }))
        .pipe(gulp.dest('work/icons/')); // サンプルhtmlの吐き出し先を指定
    })
...
gulp.task('watch-files', function(done) {
  gulp.watch('work/scss/**/*.scss', gulp.task('sass'));
  gulp.watch('work/image/**', gulp.task('image'));
  gulp.watch('work/icons/svg/*.svg', gulp.task('iconfonts'));
